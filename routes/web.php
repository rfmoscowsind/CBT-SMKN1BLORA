...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/monitoring/stats', function() {
        ...
        $pgbouncerOverview = function (): array {
            $config = config('database.connections.'.config('database.default'), []);
            $snapshot = \Illuminate\Support\Facades\Cache::get('monitoring:pgbouncer:overview');

            if (is_array($snapshot) && isset($snapshot['status'])) {
                return $snapshot;
            }
            ...
            return [
                ...
                'total_queries' => null,
                'total_xacts' => null,
                'message' => 'Snapshot PgBouncer belum tersedia.',
                'refreshed_at' => null,
            ];
        };
